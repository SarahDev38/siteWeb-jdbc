...
<script type="text/javascript" src='<c:url value="/inc/js/listeProduitsPourModification.js" />' ></script>
...
<div class="content">
	...
	<fieldset style ="max-width:1200px;">
		<legend>catégorie : ${rubrique.nom}</legend>
		<c:forEach items="${listeGammes}" var="gam">
			<c:forEach items="${listeProduits}" var="produit">
			<c:forEach items="${listeReferences}" var="ref">
			<c:if test="${produit.reference.id == ref.id && ref.rubrique.nom == rubrik.nom && ref.gamme_id == gam.id}">
				<div class="divProduit" style="float:left;width : 380px;border:1px solid black;margin:3px;">
					...
					<table style="font-size: 12px; ">
						...
						<tr><td rowspan=3  width=160px style="text-align: center;">
								<img src="${empty ref.image ? '' :'DisplayImage?image='}${ref.image}" alt="image" style="border: 1px solid black;max-width: 150px;max-height: 100px;">
							</td>
