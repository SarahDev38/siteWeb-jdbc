...
@WebServlet(urlPatterns = "/modifierProduit", initParams = {
...
public class ModificationProduit extends HttpServlet {
	...
	private static final String VUE_FORM = "/restreintGestionnaire/modifierProduit.jsp";
	private static final String VUE_LISTE = "/listeProduitsPourModification.jsp";
	@EJB
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (produit != null) {
			...
			request.setAttribute(ATT_RUBRIQUE, reference.getRubrique() );
			Gamme gamme = null;
			try {
				gamme = reference.getGamme();
			} catch (Exception e) {
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Produit produit = formProduit.modifierProduit(request, reference);
	...
	private Produit getProduitFromParameter(HttpServletRequest request) {
		...
		Long id = (long) 0;
		try {
			id = Long.parseLong(request.getParameter(PARAM_PRODUIT_ID));
		} catch (NumberFormatException e) {
		}
		if (!id.equals((long) 0)) {
			try {
				produit = produitDao.trouverId(id);
	...
	private List<Reference> setCheminsImages(List<Reference> references) {
		...
		while (iterator.hasNext()) {
			...
			reference.setImage(cheminStockage.concat(reference.getImage()));
		}
		return references;
	}
}
